// black_hole.cpp
// By: Sam Schmitz
// Created a 2d black hole by following the tutorial from kavan on YouTube
...
struct Ray {
    ...
    void draw(const std::vector<Ray>& rays) {
        ...
        for (const auto& ray : rays) {
            ...
            for (size_t i = 0; i < N; ++i) {
                // older points (i=0) get alphaL0, newer get alphaL1
    ...
    void step(double dL, double rs) {
        // 1) integrate (r,L,dr,dL)
...
void rk4Step(Ray& ray, double dL, double rs) {
    ...
    ray.r += (dL / 6.0) * (k1[0] + 2 * k2[0] + 2 * k3[0] + k4[0]);
    ray.phi += (dL / 6.0) * (k1[1] + 2 * k2[1] + 2 * k3[1] + k4[1]);
    ray.dr += (dL / 6.0) * (k1[2] + 2 * k2[2] + 2 * k3[2] + k4[2]);
    ray.dphi += (dL / 6.0) * (k1[3] + 2 * k2[3] + 2 * k3[3] + k4[3]);
}


int main() {
    // This ray does 3 loops around the black hole before flying off
    //rays.push_back(Ray(vec2(-1e11, 3.27606302719999999e10), vec2(c, 0.0f)));

    // These rays come from the left side of the screen
    for (double i = -8; i <= 8; i += 0.3) {
        rays.push_back(Ray(vec2(-1e11, i * (1e10)), vec2(c, 0.0f)));
    }
